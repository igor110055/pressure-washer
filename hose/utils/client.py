...
class FtxWebsocketClient(WebsocketManager):
    ...
    def __init__(self) -> None:
        ...
        self._api_key = 'NCfCpoN-hX1iGvvoNjdNaLAbzoLmhMraC2MhxT_M'    # TODO: place your API key here
        self._api_secret = 'f19EPgm-1GSp2owUHDy2owJKxCF9xQbzVwWyjoEO' # TODO: place your API secret key here
    ...
    def _reset_data(self) -> None:
        ...
        self._last_recieved_orderbook_data_at: float = 0.0
    ...
    def _login(self) -> None:
        """Send log in JSON, and set `_logged_in = True`"""
    ...
    def _subscribe(self, subscription: Dict) -> None:
        """Send a new subscription, and append to `_subscriptions`"""
        self.send_json(
            {
                'op': 'subscribe',
                **subscription
    ...
    def _unsubscribe(self, subscription: Dict) -> None:
        """Send an ubsubscription, then unsubscribe"""
        self.send_json(
            {
                'op': 'unsubscribe',
                **subscription
    ...
    def get_fills(self) -> List[Dict]:
        """Send a subscription to capture a list of order fills"""
    ...
    def get_orders(self) -> Dict[int, Dict]:
        """Send a subscription to capture a dictionary of orders"""
        if not self._logged_in:
            self._login()
    ...
    def get_trades(self, market: str) -> List[Dict]:
        """Send a subription to get a list of trades in a given market"""
    ...
    def get_orderbook(self, market: str) -> Dict[str, List[Tuple[float, float]]]:
        """Send subscription for orderbook data, and return a sorted dictionary of L2 data"""
    ...
    def wait_for_orderbook_update(self, market: str, timeout: Optional[float]) -> None:
        """Subscribe and wait for orderbook updates"""
        subscription = {'channel': 'orderbook', 'market': market}
        if subscription not in self._subscriptions:
            self._subscribe(subscription)
    ...
    def get_ticker(self, market: str) -> Dict:
        """Subscribe to a ticker and update tickers"""
    ...
    def _handle_orderbook_message(self, message: Dict) -> None:
        ...
        # build in fault tolerance for failed checksums
        computed_result = int(zlib.crc32(':'.join(checksum_data).encode()))
        if computed_result != checksum:
            self._last_recieved_orderbook_data_at = 0
            self._reset_orderbook(market)
